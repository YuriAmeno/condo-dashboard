...
-- Create new policies for managers
CREATE POLICY "Enable public read access to managers"
  ON managers FOR SELECT
  USING (true);

CREATE POLICY "Enable public insert access to managers"
  ON managers FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Enable managers to update their own data"
...
-- Update doormen table to link with auth.users
DO $$ BEGIN
  ALTER TABLE doormen
...
-- Create or replace function to handle user registration
...
-- Create or replace trigger for login tracking

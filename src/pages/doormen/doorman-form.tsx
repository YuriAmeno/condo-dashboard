...
export function DoormanForm() {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { user } = useAuth();
  const [isCreating, setIsCreating] = useState(false);

  const form = useForm<DoormanFormData>({
    resolver: zodResolver(doormanSchema),
    defaultValues: {
      name: '',
      cpf: '',
      email: '',
  ...
  const handleCreateDoorman = async (data: DoormanFormData) => {
    try {
      setIsCreating(true);
      ...
      // 2. Criar registro do porteiro
      const { error: doormanError } = await supabase
        .from('doormen')
        .insert({
          name: data.name,
          cpf: data.cpf,
      ...
      if (doormanError) {
        ...
        console.error('Erro ao criar porteiro:', doormanError);
        throw new Error('Erro ao criar porteiro. Por favor, tente novamente.');
      }
      ...
      console.error('Error creating doorman:', error);
      toast({
        variant: 'destructive',
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Você não tem permissão para cadastrar porteiros.
          </AlertDescription>
        </Alert>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Cadastre um novo porteiro no sistema.
        </p>
      </div>

      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleCreateDoorman)} className="space-y-6">
          {/* Dados Pessoais */}
